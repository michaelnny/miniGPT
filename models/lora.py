# Original code from https://github.com/Lightning-AI/lit-llama/blob/main/lit_llama/lora.py
# made small changes to support GPT-2 models
...
from . import gpt2_model as gpt2
...
class CausalSelfAttention(gpt2.CausalSelfAttention):
    lora_config = None

    def __init__(self, config: gpt2.GPT2Config) -> None:
        ...
        *Instead of creating multiple heads and concatenating the result (in addition to creating separate matrices for
        ...
        Args:
            config:
                ...
                ``"num_layers"``: number of transformer blocks (self-attention + MLP),
                ``"num_attn_heads"``: number of heads in multi-head attention mechanism,
                ``"hidden_size"``: size of the embedding: vector representation of each token.
        """
...
def lora(r, alpha, dropout, enabled: bool = True):
    ...
    causal_self_attention = gpt2.CausalSelfAttention
    gpt2.CausalSelfAttention = CausalSelfAttention
    yield
    # when exiting context manager - restore link to original causal self-attention class

    gpt2.CausalSelfAttention = causal_self_attention
